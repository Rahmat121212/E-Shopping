...
import { useFetchBrandQuery, useUpdateBrandImageMutation } from "../../store/services/brandService";
const UpdateBrandImage = () => {
  const {id} = useParams();
  console.log("id,.,.",id);
  const {data:cate, isFetching} = useFetchBrandQuery(id);
    console.log('Brand data: ',cate?.brand?.image)
...
console.log("Image",state);
  const [saveCategory, response] = useUpdateBrandImageMutation();
    ...
    <Wrapper>
      ...
      <div className="flex flex-wrap -mx-3">
        ...
        <div className="w-full xl:w-4/12 p-3">
          <ImagesPreview url={`/uploads/brand/${cate?.brand?.image}`} heading="Previous Image" />
